...
- **D-04 — The MOAT is the *intersection* of (a) authentic/grounded/in-your-own-voice writing + (b) norm-correct German format. Translation is an OPTIONAL feature, NOT the USP — never position the product as "we translate your CV to German."** Lead with "authentic + norm-correct, wins the recruiter"; keep translation invisible. The reason the target is *internationals applying in Germany* (not locals/natives) is precisely that the moat is the intersection: it only fully fires for people who lack BOTH the norm-knowledge AND native fluency. For a German native, (b) is table stakes (zero gap), so the product would collapse to just (a) — the most crowded, copyable space, owned by incumbents (e.g. Bewerbung.ai ~1M users) where the solo founder has no distribution edge. So: **the landing surface is audience-neutral (a local CAN use it — don't exclude them), but acquisition/SEO/community/founder-story is optimized for internationals.** Do NOT make locals/natives a co-equal target. The two-column landing (authentic | norm-correct) is the validated centerpiece (D-09), unified by the "8-second scan" frame.
- **D-05 — Phase 4 is messaging/design ONLY — zero backend changes.** The parse and cover-letter pipelines already convert any résumé regardless of nationality; bilingual German+English output already fits because English is the lingua franca of the whole inflow. No new features, no per-nationality logic, no non-English UI *in Phase 4*. (See Deferred: German-OR-English output is a separate near-term product item that DOES need backend work — not this phase.)
...
**Near-term product item (NOT Phase 4 — needs backend/prompt/UI work, but on-thesis):**
- **German OR English output, translation optional.** Let the user choose the output language: a norm-correct German *format* CV with English content serves the large English-language-role-in-Germany sub-segment without leaving the niche. Decided in principle (Phase 4 discussion) but deferred — the current product outputs German; this needs prompt/schema/UI changes and likely its own phase. Phase 4 landing copy stays honest to current capability (German output) while leading with "authentic + norm-correct" (not "translation").
