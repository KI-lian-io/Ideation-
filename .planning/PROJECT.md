...
| Moat = intersection of (a) authentic/grounded voice + (b) norm-correct German format. Translation is an OPTIONAL feature, NOT the USP. Lead with "authentic + norm-correct, wins the recruiter" under the "8-second scan" frame | Both USPs only fire together for people lacking norm-knowledge AND native fluency = internationals; for natives, (b) is table stakes so it collapses to (a) — a red ocean owned by incumbents (Bewerbung.ai ~1M) with no founder distribution edge. Surface is audience-neutral (locals may use it) but acquisition targets internationals. Authentic = quality/voice, never detection-evasion; no outcome/income claims | — Pending (Phase 4) |
| German OR English output, translation optional (near-term product item, post-Phase-4) | A norm-correct German-*format* CV with English content serves the English-role-in-Germany sub-segment without leaving the niche; needs backend/prompt/UI work — own phase | — Deferred / near-term |
