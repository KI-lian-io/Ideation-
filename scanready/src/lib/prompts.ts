...
/** Shared typography rule: no em-dash in any generated output, in any language. */
const TYPOGRAPHY_RULES = `TYPOGRAPHY:
- Never use the em-dash character "—" anywhere in your output. Use commas, colons, or
  periods instead. The en-dash "–" is allowed only for date ranges (e.g. "03/2024 – heute").`;
...
- normGapNotes: for each structural or content change made, write ONE note as an object
  with an "en" and a "de" field, both describing the SAME change (the "de" field is a
  natural, native German phrasing, not a word-for-word translation of the "en" field).
  Format for both languages: "[Change made]: [why this matters in the German recruiter's
  first scan]". Example en: "Added personal-data block: recruiters look for name, address,
  phone in the top section." Example de: "Block mit persönlichen Daten ergänzt: Recruiter
  suchen im ersten Blick gezielt nach Name, Adresse und Telefonnummer." Be concise. Do not
  repeat content already visible in the Lebenslauf.
...
- photoAdvice: an object with an "en" and a "de" field, both explaining that a photo is
  legally optional under the AGG but expected by most German recruiters; let the user
  decide. Do not fabricate a photo. The "de" field is natural German, not a translation
  of the "en" field.
 ...
 * Bilingual: `en`/`de` are natural translations of the SAME question, shown per the
 * active UI language (src/lib/i18n.tsx). buildCoverLetterUser always embeds the
 * ENGLISH text below (see that function's `qa` construction, driven by the `answers`
 * state which stores `{id, answer}` synced against these ids): the prompt sent to the
 * model is deterministic regardless of which language the applicant answered in, and
 * the model writes German regardless of the language the question was asked in.
 */
export const PERSONALIZATION_QUESTIONS: { id: string; en: string; de: string }[] = [
  {
    id: "achievement",
    en: "Describe one specific achievement from your last role that you're proud of (with a number if you can).",
    de: "Beschreiben Sie eine konkrete Leistung aus Ihrer letzten Position, auf die Sie stolz sind (mit einer Zahl, wenn möglich).",
  },
  {
    id: "why-company",
    en: "What specifically about THIS company or role made you apply?",
    de: "Was hat Sie konkret an DIESEM Unternehmen oder dieser Stelle zur Bewerbung bewogen?",
  },
  {
    id: "motivation",
    en: "What's your main motivation for working in Germany / this position right now?",
    de: "Was ist Ihre Hauptmotivation, gerade jetzt in Deutschland bzw. in dieser Position zu arbeiten?",
  },
  {
    id: "working-style",
    en: "How would colleagues describe your working style in a sentence?",
    de: "Wie würden Kolleginnen und Kollegen Ihren Arbeitsstil in einem Satz beschreiben?",
  },
  {
    id: "gap-or-change",
    en: "Any gap, career change, or unusual background you'd like to address up front?",
    de: "Gibt es eine Lücke, einen Quereinstieg oder einen ungewöhnlichen Werdegang, den Sie von vornherein ansprechen möchten?",
  },
];
...
export const SALARY_QUESTION = {
  id: "salary",
  en: 'This posting asks for a salary expectation (Gehaltsvorstellung). What gross annual figure or range do you want to state? (e.g. "55.000-60.000 € brutto/Jahr")',
  de: 'Diese Stellenanzeige fragt nach einer Gehaltsvorstellung. Welchen Bruttojahresbetrag oder welche Spanne möchten Sie angeben? (z. B. "55.000-60.000 € brutto/Jahr")',
};
export const START_DATE_QUESTION = {
  id: "start-date",
  en: 'This posting asks for your earliest start date (Eintrittstermin). When can you start? (e.g. "zum 01.09.2026" or "ab sofort")',
  de: 'Diese Stellenanzeige fragt nach Ihrem frühestmöglichen Eintrittstermin. Wann können Sie anfangen? (z. B. "zum 01.09.2026" oder "ab sofort")',
};
 ...
 * posting change, preserving answers by question id.
 */
export function questionsForPosting(
  jobPosting: string
): { id: string; en: string; de: string }[] {
