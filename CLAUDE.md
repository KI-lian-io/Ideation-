...
- **design-sync follow-up:** ✅ done — the full DesignSync export already exists locally at `./ScanReady Design System/` (`tokens/`, `components/`, `templates/`, `guidelines/`, `SKILL.md`) and has already been reconciled into the live app (commits `7ea7b6d`, `65a10f3`). No pending push action. The in-app preview library at [`scanready/design-system/`](scanready/design-system/) (`overview.html` + README, `@dsCard` markers) remains useful as a reference, not a to-do.
