...
3. **Stage 3 activation — PARTIALLY DONE (2026-07-06, live founder session)**: ✅ Supabase EU project created (Frankfurt, ref `thgmhbzimnjcaoqyyiyp`), ✅ migration applied + security-advisor-verified (function-grant hardening applied live AND in the repo migration after the advisor caught a cross-user RPC vector), ✅ Google OAuth configured and tested (sign-in works, `handle_new_user` trigger verified: profiles row created), ✅ local `scanready/.env.local` has URL + publishable key (NOTE: value goes under our var name `NEXT_PUBLIC_SUPABASE_ANON_KEY`; new-style `sb_publishable_...` keys are drop-in). ✅ **Accounts E2E passed (2026-07-06)**: save 1 package, 2nd save blocked by the DB limit trigger with the correct UI hint, reload + load round-trip verified live through the real UI against the Frankfurt project; SQL probes confirmed cross-user RLS, the read_only downgrade contract, and `delete_own_account()` cascades (method + full results in `scanready/docs/stage3-accounts.md`). Found during E2E: blocked saves leaked an orphaned `cvs` row (fixed in `895e03d`); Supabase email provider is enabled while the UI offers only Google (disable in dashboard if Google-only is intended). ⏳ Remaining: Vercel env vars (Preview first, Production last), Stripe subscription Price (3,99–5,99 € decision) + `STRIPE_WEBHOOK_SECRET` + `SUPABASE_SERVICE_ROLE_KEY`, §312k/AGB/Datenschutz legal review on a preview deploy.
4. Deferred: .docx, LinkedIn import, ATS scoring, `lang="en"`→`de` follow-up, TT Phase 3 (founder assets).

**Next build (design ready, 2026-07-07):** account content library (CV gallery / saved Anschreiben / posting history) + honest pricing ladder (keep one-time SKUs, add a non-renewing 30-day Bewerbungsphase-Pass at 14,99 €, subscription later). PRD: `scanready/docs/prd-account-library-pricing.md`. claude.ai/design export reconciled + planned: `scanready/docs/design-impl-plan-library-pricing.md` (surface→component map, phased build order). Fresh-session `/goal` handover ready at `scanready/docs/HANDOVER-library-pricing.md`. Design source committed under `scanready/claude design files/` (zips gitignored). Founder decisions still open in PRD §9.
